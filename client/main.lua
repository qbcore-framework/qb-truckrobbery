...
Citizen.CreateThread(function()
    while true do
		...
		if dist <= 25.0 and PlayerJob.name ~= 'police' then
		...
		if dist <= 1.5 and PlayerJob.name ~= 'police' then
			DrawText3D(Config.MissionMarker.x, Config.MissionMarker.y, Config.MissionMarker.z, Lang:t("mission.Accept_Mission"))
			if IsControlJustPressed(0, 38) then
				TriggerServerEvent("truckrobbbery:AcceptMission")
				Wait(500)
			end
		end
	end
...
function AlertPolice()
	...
	TriggerServerEvent('truckrobbbery:GiveInfoPolice', AlertCoordA, AlertCoordB, AlertCoordC)
	Wait(500)
end

RegisterNetEvent('truckrobbbery:InfoForPolice')
AddEventHandler('truckrobbbery:InfoForPolice', function(x, y, z)
	if PlayerJob ~= nil and PlayerJob.name == 'police' then
		...
		if PoliceBlip == 0 then
			...
			AddTextComponentString(Lang:t('info.PoliceBlip'))
		...
		if PoliceDist <= 4.5 then
			...
			if SilenceAlarm == 0 then
				hintToDisplay(Lang:t('info.Silence_alarm'))
				SilenceAlarm = 1
			end
			if IsControlPressed(0, 47) and GuardsDead == 1 then
				TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
				TriggerEvent('truckrobbbery:CleanUp')
...
AddEventHandler('truckrobbbery:client:911alert', function()
	if PoliceAlert == 0 then
		...
		if street2 ~= nil then
			...
			TriggerServerEvent("truckrobbbery:server:callCops", streetLabel, transCoords)
...
RegisterNetEvent('truckrobbbery:client:robberyCall')
AddEventHandler('truckrobbbery:client:robberyCall', function(streetLabel, coords)
    if PlayerJob.name == "police" then
        local store = "Armored Truck"

        PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
		TriggerServerEvent('police:client:policeAlert', coords, Lang:t('info.alerttitle'))
...
RegisterNetEvent('truckrobbbery:StartMission')
AddEventHandler('truckrobbbery:StartMission', function()
	MissionNotification()
...
Citizen.CreateThread(function()
    while true do
		...
		if MissionStart == 1 then
			...
			if dist <= 75.0 and not PlayerJob.name == 'police' then
			...
			if dist <= 7 and BlownUp == 0 and not PlayerJob.name == 'police' then
				if BlowBackdoor == 0 and GuardsDead == 1 then
					hintToDisplay(Lang:t('info.Detonate_bomb'))
					BlowBackdoor = 1
				end
				if IsControlPressed(0, 47) and GuardsDead == 1 then	
					CheckVehicleInformation()
					TriggerEvent("truckrobbbery:client:911alert")
					Wait(500)
				end
			end
		else
...
Citizen.CreateThread(function()
    while true do
		...
		if lootable == 1 then
			...
			if dist <= 4.5 and not PlayerJob.name == 'police' then
				if PickupMoney == 0 then
					hintToDisplay(Lang:t('info.Take_money'))
					PickupMoney = 1
				end
				if IsControlJustPressed(0, 38) then
					lootable = 0
...
RegisterNetEvent('truckrobbbery:CleanUp')
AddEventHandler('truckrobbbery:CleanUp', function()
	PickupMoney = 0
	BlowBackdoor = 0
	SilenceAlarm = 0
	PoliceAlert = 0
...
function TakingMoney()
	...
	while GetGameTimer() - _time < 20000 do
		...
		hintToDisplay(Lang:t('info.Bail_out'))
		Wait(1)
	end
	...
	TriggerServerEvent("truckrobbbery:RobberySucess", LootTime)
	TriggerEvent('truckrobbbery:CleanUp')
	Wait(2500)
end
